/* UTENDAJI NA MAWASILIANO YA MFUMO WOTE (FIREBASE FIRESTORE ENGINE) - IKA MICROFINANCE */
...
// Firebase Configuration & Initialization for IKA MICROFINANCE
const firebaseConfig = {
  apiKey: "AIzaSyC2t6UpwpeK17GGxleg18s8kzPeQ4ogbiw",
  authDomain: "ika-microfinance.firebaseapp.com",
  projectId: "ika-microfinance",
  storageBucket: "ika-microfinance.firebasestorage.app",
  messagingSenderId: "121125396587",
  appId: "1:121125396587:web:14909256f68ce6f02ad12e",
  measurementId: "G-ED0B80Z07B"
};
...
function handleUserLogin(e) {
    ...
    currentUser = found;
    addLog(`Afisa ameingia kwenye mfumo wa IKA Microfinance`, currentUser.username);
    startSession();
}
...
function handleAdminLogin(e) {
    ...
    if (u === 'admin' && p === 'admin123') {
        sessionStorage.setItem('ika_admin_active', 'true');
        addLog(`Admin ameingia kwenye Admin Panel ya IKA Microfinance`, 'admin');
...
function adminLogout() {
    sessionStorage.removeItem('ika_admin_active');
...
function saveClient(e) {
    ...
    const clientId = 'IKA-C-' + Math.floor(100 + Math.random() * 900);
    const newClient = {
        id: clientId,
    ...
    db.collection('clients').doc(clientId).set(newClient)
        .then(() => {
            addLog(`Mteja mpya amesajiliwa IKA Microfinance: ${newClient.name} (${newClient.id})`);
...
function submitLoan(e) {
    ...
    const loanId = 'IKA-L-' + Math.floor(1000 + Math.random() * 9000);
...
function submitRepayment(e) {
    ...
    const repaymentId = 'IKA-R-' + Date.now();
    const repaymentData = {
        id: repaymentId,
        loanId: loanId,
        ...
        adminLoansTable.innerHTML = `
            <tr>
                <td colspan="7" class="p-4 text-center text-slate-500 italic">
                    Hakuna taarifa za mikopo zilizopatikana kwenye IKA Microfinance.
                </td>
            </tr>`;
        return;
    }
